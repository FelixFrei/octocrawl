...
export async function scrape(startUrlArg, opts = {}) {
  ...
  if (saved) {
    ...
    for (const u of allowed) enqueue(u);
  ...
  const urlToFilePath = (u) => {
    ...
    if (p.search) {
      const suffix = p.search.slice(1).replace(/[^a-zA-Z0-9._-]/g, '_').slice(0, 100);
      rel = rel.replace(/(\.[a-z0-9]+)$/i, `__${suffix}$1`);
    }
  ...
  const stop = async () => {
    ...
    await saveState(statePath, snapshotState());
